...
class ServerUserEndpointTest(TestCase):
    ...
    def test_users_endpoint_success(self, mock_response):
        key = 'github_users_service__GitHubCommit_teradici/deploy_2019-06-01T00:00:00+0000_2020-05-01T00:00:00+0000'
        mock_response.return_value.set(key, dumps(commits), timedelta(minutes=60))
    ...
    def test_users_endpoint_error(self, mock_response):
        response = self.client.get('/users')
        data = response.json()
